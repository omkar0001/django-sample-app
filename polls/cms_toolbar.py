...
class PollToolbar(CMSToolbar):
    ...
    def populate(self):
        if self.is_current_app:
            menu = self.toolbar.get_or_create_menu('poll-app', _('Polls1'))
...
class IconExtensionToolbar(CMSToolbar):
    def populate(self):
        ...
        if has_global_current_page_change_permission or can_change:
            ...
            try:
                if icon_extension:
                    url = reverse('admin:polls_iconextension_change', args=(icon_extension.pk,))
                else:
                    url = reverse('admin:polls_iconextension_add') + '?extended_object=%s' % self.page.pk
